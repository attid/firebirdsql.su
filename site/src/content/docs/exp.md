...
## Описание
Функция EXP() возвращает результат возведения константы Эйлера в степень < аргумент >, т.н "экспоненту числа".

Константа Эйлера равна
...
Примечание: Вызов функции с аргументом, равным единице вернет значение самой константы Эйлера.
...
DECLARE EXTERNAL FUNCTION EXP
    ...
    DOUBLE PRECISION BY DESCRIPTOR
RETURNS PARAMETER 2
ENTRY_POINT 'exp' MODULE_NAME 'fbudf';
```
...
DECLARE EXTERNAL FUNCTION EXP
    ...
    DOUBLE PRECISION BY DESCRIPTOR
RETURNS PARAMETER 2
ENTRY_POINT 'exp' MODULE_NAME 'SYSTEM';
```

## Пример
```sql
SELECT EXP(0), EXP(1) FROM RDB$DATABASE
```

## См. также
UDF, [Встроенные функции](/vstroennye_funkcii/), [POWER()](/power/)
